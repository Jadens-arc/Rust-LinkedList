...
impl Node {
    ...
    fn get_recursive(&self, target: usize, cur: usize) -> Result<i32, String> {
        ...
        match self.next {
            ...
            Address::Nil => {
                return Err(format!("Index {} not found", target))
            }
        }
    }

    fn get(&self, index: usize) -> Result<i32, String> {
        self.get_recursive(index, 0)
    }

    fn remove_recursive(&mut self, target: usize, cur: usize) -> i32 {
        if target - 1 == cur {
...
fn main() {
    ...
    println!("{}", n.remove(2));
    println!("{}", n.to_string());
    println!("{}", n.get(5).expect("couldn't find"));
}
